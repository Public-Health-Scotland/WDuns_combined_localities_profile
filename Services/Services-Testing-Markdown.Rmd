---
title: ''
output:
  word_document:
    reference_docx: //conf/LIST_analytics/West Hub/02 - Scaled Up Work/RMarkdown/Locality
      Profiles/Master RMarkdown Document & Render Code/Locality_Profiles_Report_Template.docx
  html_document:
    df_print: paged
---


```{r setup, include = FALSE}
knitr::opts_chunk$set(echo = TRUE)
options(kableExtra.auto_format = FALSE)

source(here("Master RMarkdown Document & Render Code", "Global Script.R"))
source(here("Services", "2. Services data manipulation & table.R"))
source(here("Services","3. Service HSCP map.R"))

x <- 1 # object for figure numbers
y <- 1 # object for table numbers
```


## Services

#### Figure `r x`: Map of GP practices by locality in `r HSCP` HSCP^2^.

\newline

```{r echo = FALSE, fig.width=6}
service_map

x <- x + 1
```


\newline

#### Table `r y`: Number of each type of service in `r locality_list[1]` Locality^2^.
\newline

```{r echo = FALSE}
knitr::kable(services_tibble[[1]], col.names = c("**Service Type**", "**Service**", "**Number**"))

y <- y + 1
```

\newline

#### Table `r y`: Number of each type of service in `r locality_list[2]` Locality^2^.
\newline

```{r echo = FALSE}
knitr::kable(services_tibble[[2]], col.names = c("**Service Type**", "**Service**", "**Number**"))

y <- y + 1
```